...
Colabノートブックの全件モードは `run-remaining-shards` を実行し、未完了shardを
順番に処理して全件完了後に最終集計する。`RUN_ALL_REMAINING = False` で単一shardに戻せる。
エラーやランタイム切断後はセットアップ・バッチ設定を確認して同じセルを再実行する。
CU残量の自動確認・補充は行わない。A100 80GB向けノートブックの初期バッチ数は72/144。
